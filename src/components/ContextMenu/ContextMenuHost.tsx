...
function createContextRunner(payload: ContextMenuPayload) {
  return createMenuActionRunner({
    ...
    refreshFileTree: () => payload.type === "fileTreeBlank" ? payload.handlers?.refreshTree?.(payload) : undefined,
    openSearch: noop,
    ...
    deleteCurrentNote: (notePayload) => notePayload.handlers?.delete?.(notePayload),
    ...
    pasteFromSelection: (selectionPayload) => selectionPayload.handlers?.paste?.(selectionPayload),
    ...
    pasteFromBlank: (blankPayload) => blankPayload.handlers?.paste?.(blankPayload),
