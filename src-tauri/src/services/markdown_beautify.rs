    ...
    AiTextRequest {
        prompt: format!(
            concat!(
                ...
                "3. 如文章没有标题的，需要在开头添加一级标题，标题内容为笔记文件名（不带扩展名）。\n",
                "4. 修改不当的转义，比如将 \\`code\\` 修正为 `code`，将 \\$variable 修正为 `$variable`。\n",
                "5. 对于明显的行内代码、代码块或公式片段，修正为适当的 Markdown 代码格式。\n",
                "6. 若无法确定安全修复方式，尽量保持原样。\n",
                "7. 返回结果必须仍是单个完整 Markdown 文档。\n\n",
                "笔记路径：{}\n\n",
                "原始 Markdown：\n{}"
            ),
            note_path,
