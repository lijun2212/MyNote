...
3. 列表：使用 `-`、`*` 或数字列表整理步骤。
4. 表格：使用 Markdown 表格语法展示结构化信息。
5. 代码：使用行内反引号或围栏代码块展示代码、配置和命令。
6. 图片：优先使用 `![说明](路径)`，粘贴图片时 MyNote 会自动保存附件并插入引用。
7. 链接和双链：使用 Markdown 链接或 Wiki 链接连接其它笔记。
8. Mermaid：使用 `mermaid` 围栏代码块绘制图表。
9. LaTeX：使用 `$...$` 写行内公式，使用 `$$...$$` 写行间公式。
10. 有限 HTML：在少量 Markdown 不方便表达的排版场景中使用白名单 HTML 标签。
