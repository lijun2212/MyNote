...
| 版本 | 日期 | 说明 |
| --- | --- | --- |
| 1.6 | 2026-06-20 | 调整 `..(password)..` 预览表现：统一显示为 6 个星号，并收敛为文中嵌入字段样式。 |
| 1.5 | 2026-06-19 | 补充 `..(password)..` 密码字段语法说明，并新增暗色只读密码框预览效果说明。 |
...
2. “标签”：用于查看标签列表和标签上下文。标签归属以 Front Matter 中的 `tags` 字段为准；正文里的 `[[#标签名]]` 只表示显式引用与导航定位，不再自动创建标签归属。
...
2. 正文里的标签引用语法是 `[[#标签名]]`。它适合在段落里显式提到某个标签，并让标签上下文能定位到这里。
3. 如果你的旧笔记仍在使用 `#标签`，可以在仓库根目录运行 `corepack pnpm migrate:tags` 先预览迁移结果，再用 `corepack pnpm migrate:tags -- --apply` 写回文件。
4. 删除标签会影响相关笔记中的标签使用，因此更适合在你确认不再需要这个标签时再操作。
5. 如果标签很多，先用一两个主标签建立稳定习惯，通常比一开始就堆很多细标签更容易维护。
...
15. 密码字段：使用 `..(内容)..` 插入一个只读的密码字段样式，适合演示账号信息或敏感字段位置。
...
如果你想在预览区展示一个密码字段样式，可以在正文中使用 `..(内容)..`。
...
1. 这是一个行内语法，适合嵌在普通句子里使用。
2. 预览区会把它渲染为只读的密码字段，采用极淡底色、细边框、文中嵌入字段风格。
3. 预览区不会显示原始密码内容，而是统一显示为 `******`，输入框宽度也固定为 6 个字符。
4. 代码片段和代码块中的 `..(内容)..` 不会被转换，仍然保持原始文本。
5. 如果内容本身需要出现 `)..` 这一组合，当前语法会提前结束，建议拆分写法或改用普通代码文本表达。
